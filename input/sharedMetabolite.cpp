...
int max_total_bacteria = 1000000000; //1 billion [10^7]
...
void updateTotalBacteria(double *Value, const map<string, int>& NumPlaces, const vector<class FBGLPK::LPprob>& vec_fluxb) {
    total_bacteria = 0;
...
double Starvation(double *Value,
    ...
    double rate = (num_places_specie >= 1) ? starvation_rate * (currentBiomass * 1) : 0;
		...
		//cout << "Starvation rate for: " << bacteriumName << " is --->  " << rate << endl;
